...
| `R2-002` | shell replacement composition | `ui-integration` | In Progress | Route unification, shell-chrome replacement, center-workspace composition replacement, and surface-chrome replacement are accepted. The next slice replaces deeper remaining old-shell product components on `/studio` and `/studio/workspace` under the `MH-077` contract. |
...
- replace remaining deeper product-route component debt beyond shell and surface chrome, especially:
  - track controls and track-head interaction surfaces
  - clip and context-menu component chrome on real routes
  - piano-roll keyboard, tool, and velocity-lane adoption on real routes
  - any still-visible legacy component behavior or overlay affecting `/studio` or `/studio/workspace`
