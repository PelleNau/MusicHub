  ...
  return (
    <div className="group/track border-b border-white/[0.04] bg-[#121318]">
      ...
      >
        {/* ── Track header ── */}
        <div
          className={`relative sticky left-0 z-10 flex shrink-0 select-none border-r border-white/6 ${
        ...
        >
          ...
          <div className={`flex min-w-0 flex-1 flex-col justify-center overflow-hidden ${ultraCompactCapture ? "min-h-[30px] gap-[1px] px-2.5 py-1.5" : compactCapture ? "min-h-[34px] gap-[2px] px-2.5 py-1.5" : "min-h-[50px] gap-[2px] px-2 py-1.5"}`}>
            ...
            <div className="flex min-w-0 items-center gap-1.5">
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  <div className="flex min-w-0 items-center gap-1.5">
                    {typeof trackIndex === "number" ? (
                      <span className="shrink-0 text-[8px] font-medium leading-none text-white/26">
                        {trackIndex}
                      </span>
                    ) : null}
                    <span
                      className={`${compactCapture ? "text-[12px] font-medium text-white/93" : "text-[10px] font-medium tracking-[0.01em] text-white/74"} truncate leading-none cursor-text hover:text-white`}
                  ...
                  <span className={`${compactCapture && ultraCompactCapture ? "hidden" : "mt-px"} flex items-center gap-1 text-[6px] uppercase tracking-[0.08em] leading-none ${compactCapture ? "text-white/24" : "text-white/20"}`}>
              ...
              {!compactCapture ? (
                ...
                >
                  ...
                  {onReorder && (
                    <>
                  <button type="button" className="h-[14px] w-[11px] flex items-center justify-center rounded-[2px] transition-colors opacity-0 group-hover/track:opacity-100 bg-foreground/[0.05] text-foreground/42 border border-foreground/10 hover:bg-foreground/14 hover:text-foreground" onClick={() => onReorder(track.id, "up")} title="Move up">
                        <ChevronUp className="h-2.5 w-2.5" />
                      </button>
                  <button type="button" className="h-[14px] w-[11px] flex items-center justify-center rounded-[2px] transition-colors opacity-0 group-hover/track:opacity-100 bg-foreground/[0.05] text-foreground/42 border border-foreground/10 hover:bg-foreground/14 hover:text-foreground" onClick={() => onReorder(track.id, "down")} title="Move down">
                        <ChevronDown className="h-2.5 w-2.5" />
                      </button>
                    </>
                  )}
                  <button type="button" className="h-[14px] w-[14px] flex items-center justify-center rounded-[2px] text-[9px] transition-colors opacity-0 group-hover/track:opacity-100 bg-foreground/[0.05] text-foreground/42 border border-foreground/10 hover:bg-destructive/20 hover:text-destructive hover:border-destructive/30" onClick={() => onDeleteTrack(track.id)} title="Delete track">
                  ...
                  {onAutomationAdd && (
                    <div className="relative">
                      <button
                        type="button"
                        className={`h-[14px] w-[14px] flex items-center justify-center rounded-[2px] text-[9px] transition-colors border ${
                          visibleLanes.length > 0
                          ? "bg-primary/20 text-primary border-primary/30"
                            : "opacity-0 group-hover/track:opacity-100 bg-foreground/[0.05] text-foreground/42 border-foreground/10 hover:bg-primary/20 hover:text-primary hover:border-primary/30"
                        }`}
                      ...
                      {showAutoMenu && (
                        <div className="absolute left-0 top-full mt-1 z-50 bg-card border border-border rounded-[4px] shadow-lg py-1 w-44 max-h-48 overflow-y-auto">
            ...
            <div className={`flex items-center gap-1.25 ${compactCapture ? "rounded-[4px] border border-white/7 bg-black/14 px-1.75 py-1.5" : "rounded-[2px] border border-white/4 bg-black/10 px-1 py-[3px]"}`}>
              {compactCapture ? (
                <div
                  data-track-control="true"
                ...
                >
                  ...
                  <button
                    type="button"
                    className="flex h-[14px] w-[14px] items-center justify-center rounded-[3px] border border-white/10 bg-[#262930] text-[6.5px] font-bold leading-none text-white/34"
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                    }}
              ...
              {compactCapture ? null : (
                <>
                  <span className="w-7 shrink-0 text-right font-mono text-[7px] leading-none tabular-nums text-white/48">{volumeToDb(track.volume)}</span>
                  <div {...hp("pan")}><PanControl value={track.pan} onChange={(p) => onPanChange(track.id, p)} /></div>
                  <span className="w-4 shrink-0 text-right font-mono text-[7px] leading-none tabular-nums text-white/48">{panToDisplay(track.pan)}</span>
                </>
              )}
              {compactCapture ? (
            ...
            {!compactCapture && !isReturn && !isMaster && returnTracks.length > 0 && (
              <div className="mt-[1px] flex items-center gap-1.5 rounded-[2px] border border-white/4 bg-black/8 px-1 py-[3px]" {...hp("sends")}>
                <span className="shrink-0 text-[6px] font-mono uppercase tracking-[0.08em] leading-none text-white/34">SND</span>
                {returnTracks.map((rt) => (
          ...
          {compactCapture ? (
            <div className="flex w-[7px] shrink-0 items-stretch justify-center py-1.5 pr-[2px]">
              <div className="relative h-full w-[2px] rounded-full bg-white/7">
                <div
                  className="absolute inset-x-0 bottom-0 rounded-full"
                  style={{
                    height: `${compactMeterPct}%`,
                    backgroundColor: color,
                    opacity: track.is_muted ? 0.22 : 0.9,
