...
import { Search, Package, Plus, Library, X, HardDrive, Cpu, ChevronRight } from "lucide-react";
...
/* ── Detail panel ── */
function DetailPanel({ item, onClose }: { item: InventoryItem; onClose: () => void }) {
  const imgSrc = item.imageUrl || getCategoryFallbackIcon(item.category, item.ecosystem);
...
/* ── Main ── */
export default function MockFlightCase() {
  ...
  const ecoFiltered = useMemo(() => {
    if (selectedEco === "All") return inventoryData;
    if (selectedEco === "Hardware") return inventoryData.filter((i) => i.ecosystem === "Hardware");
    return inventoryData.filter((i) => i.ecosystem !== "Hardware");
  }, [selectedEco]);
  ...
  const soundCategoryCounts = useMemo(() => {
    ...
    categoryFiltered.forEach((i) => {
      if (i.soundCategory) map.set(i.soundCategory, (map.get(i.soundCategory) || 0) + 1);
    });
    return map;
  }, [categoryFiltered]);

  const soundCategories = useMemo(() => Array.from(soundCategoryCounts.keys()).sort(), [soundCategoryCounts]);
  ...
  const filteredItems = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      items = items.filter((i) =>
        [i.product, i.vendor, i.category, i.type, i.sonicRole, i.keywords, i.description]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
          .includes(q)
    ...
    return items;
  }, [categoryFiltered, selectedSoundCategory, searchQuery]);

  const ecoCounts = useMemo(() => ({
    All: inventoryData.length,
    Hardware: inventoryData.filter((i) => i.ecosystem === "Hardware").length,
    Software: inventoryData.filter((i) => i.ecosystem !== "Hardware").length,
  }), []);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <Glass className="mx-4 mt-4 mb-3 !rounded-xl px-4 py-3 flex flex-wrap items-center justify-between gap-3" staggerIndex={0}>
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-[10px] text-muted-foreground">{filteredItems.length} / {inventoryData.length}</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button className="h-8 px-3 rounded-lg bg-white/[0.06] border border-white/[0.06] font-mono text-[10px] text-muted-foreground hover:text-foreground hover:bg-white/[0.10] transition-all flex items-center gap-1.5">
            <Library className="h-3 w-3" /> Import
          </button>
          <button className="h-8 px-3 rounded-lg bg-primary/20 border border-primary/30 font-mono text-[10px] text-primary hover:bg-primary/30 transition-all flex items-center gap-1.5">
      ...
      <div className="flex flex-1 overflow-hidden px-4 pb-4 gap-3">
        {/* Sidebar */}
        <Glass className="w-52 shrink-0 flex flex-col !rounded-xl" staggerIndex={1}>
          {/* Eco tabs */}
          ...
          <div className="p-3 border-t border-white/[0.06]">
            <div className="flex justify-between font-mono text-[10px] text-muted-foreground/60">
              ...
              <span>{inventoryData.length} total</span>
            </div>
          </div>
        </Glass>
